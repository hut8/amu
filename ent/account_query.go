...
func (aq *AccountQuery) QueryMailboxes() *MailboxQuery {
	query := &MailboxQuery{config: aq.config}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withMailboxes: aq.withMailboxes.Clone(),
...
func (aq *AccountQuery) WithMailboxes(opts ...func(*MailboxQuery)) *AccountQuery {
	query := &MailboxQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withMailboxes = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context) ([]*Account, error) {
	...
	if query := aq.withMailboxes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Mailboxes = []*Mailbox{}
		}
		query.withFKs = true
		query.Where(predicate.Mailbox(func(s *sql.Selector) {
			s.Where(sql.InValues(account.MailboxesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.account_mailboxes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_mailboxes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_mailboxes" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Mailboxes = append(node.Edges.Mailboxes, n)
		}
	}
